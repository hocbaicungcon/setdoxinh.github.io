...
const polish=document.createElement('style');polish.textContent='.search{position:relative}.search:before{box-shadow:none;transform:translate(-3px,-3px)}.search:after{content:"";position:absolute;width:8px;height:2px;border-radius:4px;background:var(--ink);transform:translate(6px,6px) rotate(45deg)}.global-sale{margin-bottom:34px!important}.post-card img{cursor:pointer}';document.head.appendChild(polish);
document.querySelectorAll('nav a').forEach(a=>{if(a.textContent.includes('Review'))a.href='posts.html?category=Review%20Thời%20Trang';if(a.textContent.includes('Cẩm Nang'))a.href='posts.html?category=Cẩm%20Nang%20Phối%20Đồ';if(a.textContent.includes('Gợi Ý'))a.href='posts.html?category=Gợi%20Ý%20Mua%20Sắm'});
